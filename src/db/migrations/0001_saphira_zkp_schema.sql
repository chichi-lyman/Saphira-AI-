...
CREATE TABLE IF NOT EXISTS zkp_verification_logs (
    ...
    tenant_id UUID NOT NULL REFERENCES tenants(tenant_id) ON DELETE CASCADE,
    document_id UUID REFERENCES corporate_documents(document_id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS swarm_agent_logs (
    ...
    tenant_id UUID NOT NULL REFERENCES tenants(tenant_id) ON DELETE CASCADE,
    parent_document_id UUID REFERENCES corporate_documents(document_id) ON DELETE CASCADE,
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'clm_integrations' AND policyname = 'clm_tenant_isolation'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'zkp_verification_logs' AND policyname = 'zkp_tenant_isolation'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'swarm_agent_logs' AND policyname = 'swarm_tenant_isolation'
    ) THEN
